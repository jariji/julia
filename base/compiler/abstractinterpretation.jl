...
function abstract_call_gf_by_type(interp::AbstractInterpreter, @nospecialize(f),
    ...
    (; rt, effects) = from_interprocedural!(rettype, all_effects, sv, arginfo, conditionals)

    if call_result_unused(sv) && !(rt === Bottom)
...
"""
    from_interprocedural!(rt, effects::Effects,
...
"""
function from_interprocedural!(@nospecialize(rt), effects::Effects,
...
function abstract_call_method_with_const_args(interp::AbstractInterpreter, result::MethodCallResult,
    ...
    # try constant prop'
    inf_result = cache_lookup(mi, arginfo.argtypes, get_inference_cache(interp))
...
function maybe_get_const_prop_profitable(interp::AbstractInterpreter, result::MethodCallResult,
    ...
    if !force && !const_prop_methodinstance_heuristic(interp, mi, result, arginfo, sv)
...
function const_prop_methodinstance_heuristic(interp::AbstractInterpreter,
    ...
    if isdefined(method, :source) && is_inlineable(method.source)
        ...
        elseif is_stmt_noinline(flag)
            ...
            return false
        elseif any_const_prop_profitable_args(result.effects, arginfo.argtypes)
            return true
        else
...
function abstract_invoke(interp::AbstractInterpreter, (; fargs, argtypes)::ArgInfo, sv::InferenceState)
    ...
    (; rt, effects) = from_interprocedural!(rt, effects, sv, arginfo, sig)
        ...
        if fargs !== nothing
            ...
            if is_call_argument(arg, sv) && !isempty(sv.ssavalue_uses[sv.currpc])
                if 1 ≤ slot_id(arg) ≤ 8
                    ...
                    merge_effects!(sv, Effects(EFFECTS_TOTAL; const_prop_profitable_args))
                end
            end
        end
    end
...
function abstract_call_opaque_closure(interp::AbstractInterpreter,
    ...
    (; rt, effects) = from_interprocedural!(rt, effects, sv, arginfo, match.spec_types)
    return CallMeta(rt, effects, info)
end
                ...
                elseif isa(stmt, GotoIfNot)
                    ...
                    if is_call_argument(condx, frame)
                        ...
                        if 1 ≤ slot_id(condx) ≤ 8
                            ...
                            merge_effects!(frame, Effects(EFFECTS_TOTAL; const_prop_profitable_args))
                        end
                    end
